...
#define calculate function pay=hour*hourrates
def computepay(h,r):
#if hour>40,time and a half for the hourrates for all hours above 40
    if h > 40 :
        pay = 40 * r + (h - 40) * 1.5 * r
        return pay
#if hour<=40,use normal calculation
    else:
        pay = h * r
        return pay
#converr input into float type
